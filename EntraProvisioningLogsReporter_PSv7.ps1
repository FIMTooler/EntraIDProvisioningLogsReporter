...
Function Append-HashTable
{
    ...
    if ($Source -is [System.Collections.IDictionary] -or $Source -is [Hashtable] -or $Source -is [System.Text.Json.Nodes.JsonObject])
    {
...
# Multiple times faster and less memory intensive
$logObj = [System.Text.Json.JsonSerializer]::Deserialize[System.Text.Json.Nodes.JsonArray]([IO.File]::OpenRead(($JsonFile | Convert-Path)))
...
switch ($PSCmdlet.ParameterSetName)
{
    ...
    'Changes'
    {
        ...
        $logObj = $logObj | where { $_['provisioningSteps'] | where { $_['provisioningStepType'] -in "export" -and $_['status'] -in "success" } }
    }
    'Failures'
    {
        # failures
        "Filtering for failures" | Write-Host
        $logObj = $logObj | where {$_['provisioningStatusInfo']['status'] -in "failure" }
    }
}
...
for ($i = 0; $i -lt $logObj.Count; $i++)
{
    ...
    try
    {
        # ***** CRITICAL NOTES *****
        # *****
        # ***** logObj[$i] is System.Text.Json.Nodes.JsonObject. Property\Key names ARE case-sensitive
        # *****
        # ***** lobObj[$i]['property\key'] does NOT reference string object, but JsonValueOfElement object
        # ***** Thus
        # ***** 1. Objects added to Hashtable are of type JsonValueOfElement, not string. They are implicitly cast to string during export to CSV
        # ***** 2. To access string value for comparison, ToString() must be called
        # ***** 3. Without ToString(), '-eq' operator for string comparision will always return $false
        # ***** 4. ALWAYS check for $null before calling ToString() to avoid error of calling method on null-valued expression
        # ***** 5. Operators such as '-in' that do implicit casts can be used in place of ToString(). Also does not require $null check
        # ***** 6. '+' operator cannot be used for string concatenation without ToString() as there is no 'op_Addition' method
        # *****    Alternative to checking for $null and calling ToString() is use expression within string - "$(lobObj[$i]['property\key'])"
        # *****

        # prep hashTable with known fields on all objects
        $updateHT = [System.Collections.Hashtable]::new(200)
        $updateHT.Add("id", $logObj[$i]['id'])
        $updateHT.Add("activityDateTime", $logObj[$i]['activityDateTime'].ToString().Replace('T', ' ').Replace('Z',''))
        ...
        if ($Detailed -and $logObj[$i]['provisioningSteps'].Where({$_['provisioningStepType'] -in "import" -and $_['name'] -in "EntryImportAdd", "EntryImport"}).Count -gt 0)
        {
            $temp = $logObj[$i]['provisioningSteps'].Where({$_['provisioningStepType'] -in "import" -and $_['name'] -in "EntryImportAdd", "EntryImport"})[0]['details']
        ...
        if ($Detailed -and $logObj[$i]['provisioningSteps'].Where({$_['provisioningStepType'] -in "matching" -and $_['name'] -in "EntryImport", "EntryImportByMatchingProperty", "EntryImportByJoiningProperty"}).Count -gt 0)
        {
            $temp = $logObj[$i]['provisioningSteps'].Where({$_['provisioningStepType'] -in "matching" -and $_['name'] -in "EntryImport", "EntryImportByMatchingProperty", "EntryImportByJoiningProperty"})[0]['details']
        ...
        foreach ($property in $logObj[$i]['modifiedProperties'])
        {
            ...
            {
                if ($logObj[$i]['provisioningSteps'].Where({$_['provisioningStepType'] -in "matching" -and $_['name'] -in "EntryImport", "EntryImportByMatchingProperty", "EntryImportByJoiningProperty"}).Count -gt 0)
                {
                    $targetDetails = $logObj[$i]['provisioningSteps'].Where({$_['provisioningStepType'] -in "matching" -and $_['name'] -in "EntryImport", "EntryImportByMatchingProperty", "EntryImportByJoiningProperty"})[0]['details']
